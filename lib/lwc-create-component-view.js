...
import tsTemplate from './templates/typescript';
...
export default class LwcCreateComponentView {
  ...
  createBundle(moduleName, componentName, includeTest, includeCss) {
    ...
    if (includeTest) {
      ...
      fs.writeFileSync(
        `${testPath}/${componentName}.test.ts`,
        testTemplate(moduleName, componentName)
      );
    }

    // stories
    const storiesPath = `${componentPath}/__stories__`;
    if (!fs.existsSync(storiesPath)) fs.mkdirSync(storiesPath);
    fs.writeFileSync(
      `${storiesPath}/${componentName}.stories.ts`,
      storiesTemplate(moduleName, componentName)
    );
